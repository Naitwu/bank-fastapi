...
echo "Error: DIGITAL_OCEAN_IP_ADDRESS environment variable is not set."
exit 1
fi
...
echo "Uploading project... Please wait."
...
cleanup_remote() {
    echo "Cleaning up remote temporary files and dir..."
...
TEMP_DIR=$(mktemp -d)
echo "Extracting project to temporary directory $TEMP_DIR..."
...
docker-compose -f "$TEMP_DIR/production.yml" up -d --build --remove-orphans
