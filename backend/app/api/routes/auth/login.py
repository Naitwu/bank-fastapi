from fastapi import APIRouter, HTTPException, status, Depends, Response
from sqlmodel.ext.asyncio.session import AsyncSession
from backend.app.core.db import get_session
from backend.app.core.logging import get_logger
from backend.app.auth.schema import LoginRequestSchema, OTPVerifyRequestSchema
from backend.app.api.services.user_auth import user_auth_service
from backend.app.core.config import settings
from backend.app.auth.utils import set_auth_cookies, create_jwt_token

logger = get_logger()

router = APIRouter(
    prefix="/auth",
    tags=["Authentication"],
)

@router.post("/login/request-otp", status_code=status.HTTP_200_OK)
async def request_login_otp(
    login_request: LoginRequestSchema,
    session: AsyncSession = Depends(get_session),
):
    try:
        user = await user_auth_service.get_user_by_email(email=login_request.email, session=session)
        if user:
            await user_auth_service.check_user_lockout(user, session)

            if not await user_auth_service.verify_user_credentials(login_request.password, user.hashed_password):
                await user_auth_service.increment_failed_login_attempts(user, session)
                remain_attempts = settings.LOGIN_ATTEMPTS - user.failed_login_attempts
                if remain_attempts > 0:
                    error_message =f"Invalid credentials. You have {remain_attempts} attempt(s) left."
                else:
                    error_message = f"Your account has been locked due to multiple failed login attempts. Please try again after {settings.LOCKOUT_DURATION_MINUTES} minutes."

                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail={
                        "status": "error",
                        "message": error_message,
                        "action": "Please check your email and password and try again.",
                        "remaining_attempts": remain_attempts
                    }
                )
            if not user.is_active:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="User account is inactive. Please contact support."
                )
            await user_auth_service.reset_user_state(user, session, clear_otp=True, log_action=True)
            await user_auth_service.generate_and_save_otp(user, session)
            return {
                "status": "success",
                "message": "If an account exists with the provided email, an OTP has been sent."
            }
    except HTTPException as http_exc:
        raise http_exc
    except Exception as exc:
        logger.error(f"Error during login OTP request: {exc}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while processing your request. Please try again later."
        )

@router.post("/login/verify-otp", status_code=status.HTTP_200_OK)
async def verify_login_otp(
    otp_verify_request: OTPVerifyRequestSchema,
    response: Response,
    session: AsyncSession = Depends(get_session),
):
    try:
        user = await user_auth_service.verify_login_otp(
            email=otp_verify_request.email,
            otp=otp_verify_request.otp,
            session=session)
        await user_auth_service.reset_user_state(user, session, clear_otp=True, log_action=True)

        access_token = create_jwt_token(user.id)
        refresh_token = create_jwt_token(user.id, type=settings.COOKIE_REFRESH_NAME)
        set_auth_cookies(response, access_token, refresh_token)

        return {
            "message": "Login successful.",
            "user": {
                "email": user.email,
                "username": user.username,
                "first_name": user.first_name,
                "last_name": user.last_name,
                "full_name": user.full_name,
                "id_no": user.id_no,
                "role": user.role,
            }
        }
    except HTTPException as http_exc:
        raise http_exc
    except Exception as exc:
        logger.error(f"Error during OTP verification: {exc}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while processing your request. Please try again later."
        )
