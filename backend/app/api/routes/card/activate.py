...
async def activate_card_route(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to send card activated email: {e}")
        ...
        logger.error(f"HTTP error during card activation: {http_exc.detail}")
        raise http_exc
    except Exception as e:
        logger.error(f"Unexpected error during card activation: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred while activating the virtual card.",
        )
