...
async def create_card_route(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to send card created email: {e}")

        return VirtualCardReadSchema.model_validate(card)
    except HTTPException as http_exc:
        raise http_exc
    except Exception as exc:
        logger.error(f"Error creating virtual card: {exc}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while creating the virtual card.",
        ) from exc
