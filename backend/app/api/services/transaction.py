...
from backend.app.transaction.enums import TransactionTypeEnum, TransactionCategoryEnum, TransactionStatusEnum, TransactionFailureReasonEnum
...
async def initiate_transfer(
    *,
    ...
    try:
        ...
        sender_stmt = select(BankAccount, User).join(User).where(BankAccount.id == sender_account_id, BankAccount.user_id == sender_id)
        ...
        receiver_stmt = select(BankAccount, User).join(User).where(BankAccount.account_number == receiver_account_number)
        ...
        sender_user.otp = otp
        sender_user.otp_expiry_time = datetime.now(timezone.utc) + timedelta(minutes=settings.OTP_EXPIRATION_MINUTES)
...
async def complete_transfer(
    *,
    ...
    try:
        ...
        if not sender_user.otp_expiry_time or sender_user.otp_expiry_time < datetime.now(timezone.utc):
            await mark_transaction_failed(
                transaction=transaction,
                reason=TransactionFailureReasonEnum.OTP_EXPIRED,
                details={"expiry_time": (
                    sender_user.otp_expiry_time.isoformat() if sender_user.otp_expiry_time else None
                ),
        ...
        sender_account.balance = float(Decimal(str(sender_account.balance)) - transaction.amount)

        receiver_account.balance = float(Decimal(str(receiver_account.balance)) + converted_amount)
